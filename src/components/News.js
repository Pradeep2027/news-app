...
export class News extends Component {
  ...
  fetchMoreData = async () => {
    const url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}${this.props.category===''?'':'&category='+this.props.category}&apiKey=17852d8d33a2434fab88742bf2dd32ee&page=${this.state.page+1}&pageSize=${this.props.pageSize}`;
    let data = await fetch(url);
    let parsedData = await data.json();
          ...
          <InfiniteScroll dataLength={this.state.articles.length} next={this.fetchMoreData} hasMore={!this.state.lastPage} loader={<Spinner />} style={{overflow: "visible"}}>
            <div className="row">
              {this.state.articles.map((a, i) => {
                return <NewsItem key={i} title={a.title} description={a.description} urlToImage={a.urlToImage} url={a.url} author={a.author} date={a.publishedAt} source={a.source.name}></NewsItem>
              })}
            </div>
          </InfiniteScroll>
        </div>
